...
export class DelegationContextManager {
  ...
  private findDeepestDelegation(
    ...
    const filteredNestedContexts = nestedContexts.filter(c => c.callId !== context.callId);

    if (filteredNestedContexts.length === 0) {
      // No nested delegation (excluding self), current context is deepest
      return { delegation: context, depth: currentDepth };
    }
    ...
    for (const nestedContext of filteredNestedContexts) {
