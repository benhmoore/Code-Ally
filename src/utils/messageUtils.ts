...
// ===========================================
// CONTEXT CHANGE REMINDERS
// ===========================================

/**
 * Create directory added reminder
 */
export function createDirectoryAddedReminder(dirPath: string): Message {
  const content = `A new directory was just added to your accessible scope: ${dirPath}\n\nYou can now read, write, grep, and use other file tools in this location. The user may want you to explore it.`;
  return createSystemReminder(content, false);
}

/**
 * Create directory removed reminder
 */
export function createDirectoryRemovedReminder(dirPath: string): Message {
  const content = `A directory was removed from your accessible scope: ${dirPath}\n\nYou can no longer access files in this location (unless it's within the current working directory).`;
  return createSystemReminder(content, false);
}
