...
export async function autoSaveTodos(todos: TodoItem[]): Promise<void> {
  try {
    ...
    const additionalDirectories =
      additionalDirsManager && typeof (additionalDirsManager as any).getAdditionalDirectories === 'function'
        ? (additionalDirsManager as any).getAdditionalDirectories()
        : undefined;

    await (sessionManager as any).autoSave(messages, todos, idleMessages, projectContext, additionalDirectories);
  } catch (error) {
