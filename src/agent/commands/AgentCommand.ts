...
export class AgentCommand extends Command {
  static readonly metadata: CommandMetadata = {
    ...
    subcommands: [
      ...
      { name: 'create', description: 'Create new specialized agent' },
      { name: 'delete', description: 'Delete agent', args: '<name>' },
      { name: 'use', description: 'Run task with agent', args: '<name> <task>' },
      { name: 'active', description: 'Show active pooled agents' },
      { name: 'stats', description: 'Show pool statistics' },
      { name: 'clear', description: 'Clear specific agent or all', args: '[id]' },
    ],
    useYellowOutput: true,
  };

  static {
    CommandRegistry.register(AgentCommand.metadata);
  }

  readonly name = AgentCommand.metadata.name;
  readonly description = AgentCommand.metadata.description;

  // Use yellow output for simple status messages
  protected readonly useYellowOutput = AgentCommand.metadata.useYellowOutput ?? false;
  ...
  private showHelp(): CommandResult {
    const meta = AgentCommand.metadata;

    // Group subcommands: specialized agents (first 5) and agent pool (last 3)
    const specializedAgents = meta.subcommands!.slice(0, 5);
    const agentPool = meta.subcommands!.slice(5);

    const specializedLines = specializedAgents.map(sub => {
      const cmd = sub.args
    ...
    const poolLines = agentPool.map(sub => {
      const cmd = sub.args
        ? `${meta.name} ${sub.name} ${sub.args}`
        : `${meta.name} ${sub.name}`;
      return `\`${cmd}\`  ${sub.description}`;
