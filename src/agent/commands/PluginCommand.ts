...
export class PluginCommand extends Command {
  static readonly metadata: CommandMetadata = {
    ...
    subcommands: [
      ...
      { name: 'deactivate', description: 'Deactivate a plugin', args: '<name>' },
    ],
    useYellowOutput: true,
  };

  static {
    CommandRegistry.register(PluginCommand.metadata);
  }

  readonly name = PluginCommand.metadata.name;
  readonly description = PluginCommand.metadata.description;

  // Use yellow output for status messages
  protected readonly useYellowOutput = PluginCommand.metadata.useYellowOutput ?? false;
  ...
  private showHelp(): CommandResult {
    const meta = PluginCommand.metadata;
