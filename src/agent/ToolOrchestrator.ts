...
export class ToolOrchestrator {
  ...
  constructor(
    ...
    console.log('[TOOL_ORCHESTRATOR] Created with parentCallId:', this.parentCallId);
  }
  ...
  async executeToolCalls(toolCalls: ToolCall[]): Promise<void> {
    console.log('[TOOL_ORCHESTRATOR] executeToolCalls called with', toolCalls.length, 'tool calls');
    ...
    console.log('[TOOL_ORCHESTRATOR] After unwrapping batch calls:', unwrappedCalls.length, 'tool calls');
  ...
  private async executeConcurrent(toolCalls: ToolCall[]): Promise<void> {
    ...
    console.log('[TOOL_ORCHESTRATOR] executeConcurrent - groupId:', groupId, 'parentCallId:', this.parentCallId, 'toolCount:', toolCalls.length);
    console.log('[TOOL_ORCHESTRATOR] Tools in group:', toolCalls.map(tc => `${tc.function.name}(${JSON.stringify(tc.function.arguments)})`).join(', '));
    this.emitEvent({
      id: groupId,
  ...
  private async processToolResult(
    ...
    console.log('[TOOL_ORCHESTRATOR] processToolResult - tool:', toolCall.function.name, 'id:', toolCall.id, 'success:', result.success, 'resultLength:', formattedResult.length);
    ...
    console.log('[TOOL_ORCHESTRATOR] processToolResult - tool result added to agent conversation');
  }
