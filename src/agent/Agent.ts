...
export class Agent {
  ...
  constructor(
    ...
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Created - isSpecialized:', config.isSpecializedAgent || false, 'parentCallId:', config.parentCallId || 'none');
    ...
    if (config.systemPrompt) {
      ...
      console.log('[AGENT_CONTEXT]', this.instanceId, 'System prompt added, length:', config.systemPrompt.length);
    }
  }
  ...
  private async getLLMResponse(): Promise<LLMResponse> {
    ...
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Sending', this.messages.length, 'messages to LLM');
  ...
  private async processToolResponse(
    ...
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Processing tool response with', toolCalls.length, 'tool calls');
    ...
    console.log('[AGENT_CONTEXT]', this.instanceId, 'After unwrapping:', unwrappedToolCalls.length, 'tool calls');
    ...
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Assistant message with tool calls added. Total messages:', this.messages.length);

    // Execute tool calls via orchestrator (pass original calls for unwrapping)
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Executing tool calls via orchestrator...');
    await this.toolOrchestrator.executeToolCalls(toolCalls);
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Tool calls completed. Total messages now:', this.messages.length);

    // Clear current turn (for redundancy detection)
    this.toolManager.clearCurrentTurn();

    // Get follow-up response from LLM
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Getting follow-up response from LLM...');
  ...
  addMessage(message: Message): void {
    ...
    console.log('[AGENT_CONTEXT]', this.instanceId, 'Message added:', message.role, toolInfo, toolCallId, toolName, '- Total messages:', this.messages.length);
  }
